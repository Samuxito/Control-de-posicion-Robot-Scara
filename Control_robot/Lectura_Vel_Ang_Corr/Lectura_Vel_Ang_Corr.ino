...
void loop() {
  if (Serial.available() > 0) {
    decicion = Serial.read();
    // Definicion de voltaje a enviar hacia el motor
    if (decicion == '1') {
      volt = 0.01;
      r1 = 0.01;
    }
  ...
  if (interruptCounter >= 1) {
    ...
    if (interruptCounter_encoder >= 1) {
      ...
      if (paso_ang * 0.45 > 2500 || -paso_ang * 0.45 < -2500) {
        paso_ang = 0;
      }
    }
    if (u <= 9 && u >= 0.0) {
    ...
    Serial.print(rpm * 60);
    Serial.print(", ");
    Serial.print(paso_ang * 0.45); // Samuel
    //Serial.print(paso_ang * 0.45); // Nicolas
    Serial.print(", ");
